...
import Accac.BaseDriver;
import Accac.HadoopUtil;
import Accac.HdfsUtil;
import Accac.JobInitModel;
import Map_Reduce.File_op;
...
/**
 * Created by xiaohei on 16/3/9. 将邻接概率矩阵和pr矩阵进行计算并将得到的pr结果输出
 */
public class CalcPeopleRank {

    /**
     * 输入邻接概率矩阵和pr矩阵 按照矩阵相乘的公式,将对应的数据输出到reduce进行计算
     */
    public static class CalcPeopleRankMapper extends Mapper<LongWritable, Text, Text, Text> {
        ...
        /**
         * k的作用是将pr矩阵的列和邻接矩阵的行对应起来 如:pr矩阵的第一列要和邻接矩阵的第一行相乘,所以需要同时输入到reduce中
         */
        @Override
    ...
    /**
     * 每行输入都是两个矩阵相乘中对应的值 如:邻接矩阵的第一行的值和pr矩阵第一列的值
     */
    ...
    public static void run() throws InterruptedException, IOException, ClassNotFoundException {
        ...
        JobInitModel job = new JobInitModel(new String[] { inPath1, inPath2 },
                                            ...
                                            null,
                                            "CalcPeopleRank",
                                            CalcPeopleRank.class,
                                            null,
                                            CalcPeopleRankMapper.class,
                                            Text.class,
                                            Text.class,
                                            null,
                                            null,
                                            CalcPeopleRankReducer.class,
                                            Text.class,
                                            Text.class);
